...
8. 除非修改要求明确要求增删、合并、拆分或调整结构，否则必须保持选区原有的标题级别、段落边界、列表层级和空行；局部措辞修改不得合并相邻段落。
9. 以“一、”“二、”等中文序号开头并独占一行的短句，默认视为条目标题。条目标题必须独立成行，不得与其后正文连写。
10. 当用户要求合并两个或多个条目但没有明确要求标题与正文连写时，应保留或合成一个独立标题，只合并标题下的正文；标题与正文之间使用一个空行分隔。
{SELECTION_EDIT_OUTPUT_INSTRUCTIONS}"""
