...
{
    "items": [
        { "id": "opinion-101", "text": "请程书记签发" },
        { "id": "opinion-102", "text": "请程记书签发" }
    ]
}
```
...
| 字段 | 必填 | 类型 | 说明 |
|------|------|------|------|
| items | 是 | array | 批量条目，1-20 条 |
| items[].id | 是 | string | 调用方自定义请求标识，响应原样带回作为 key，同一批内不能重复 |
| items[].text | 是 | string | 待审查文本，不能为空 |
...
`review` 内的字段与单条文本审查（第 4 节）完全一致，此处展示时省略部分字段。

#### 条目结果字段

| 字段 | 类型 | 说明 |
|------|------|------|
| success | boolean | 该条是否审查成功 |
| error | string | 失败原因，成功时为空字符串 |
| code | string/null | 失败编码：VALIDATION（文本为空等）、GUARD_UNAVAILABLE（审查服务不可用）、INTERNAL（服务内部错误）；成功时为 null |
| review | object/null | 审查结果（字段同第 4 节）；失败时为 null |

#### 错误响应

| 状态码 | 说明 |
|--------|------|
| 422 | items 为空 / 超过 20 条 / id 重复 / 条目缺字段 |
