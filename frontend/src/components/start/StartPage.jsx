...
function StartPage({ currentSession, onGenerate, isGenerating }) {
  ...
  const [referenceWriteType, setReferenceWriteType] = useState('reference')
          ...
          <>
            ...
            <section className="start-section">
              ...
              <div className="template-options-grid three-col">
                {[
                  { value: 'reply', label: '生成回函', desc: '识别来文事项，逐项生成正式回复' },
                  { value: 'imitate', label: '底稿微调', desc: '选定主底稿，保留原文结构并局部修改' },
                  { value: 'reference', label: '智能参考写作', desc: '分析多份材料用途，综合生成新文稿' },
                ].map(t => (
                  <div
                    key={t.value}
            ...
            <section className="start-section">
              ...
              <textarea
                ...
                className="start-textarea"
                placeholder="例如：以第二份为主底稿，只更新届次和日期；第一份补充新增事项，第三份只参考表达…"
                rows={3}
                disabled={isGenerating}
              />
            </section>
