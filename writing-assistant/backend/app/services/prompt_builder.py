...
# 从本地北航公文语料提炼的整体语言画像，不把统计结果变成逐句模板。
STYLE_LANGUAGE_RULE = """【整体语言风格（北航公文语域）】
文章整体应贴近正式高校行政公文的语言气质：准确、克制、简洁、稳健，先完整表达事实、关系、任务和责任，再追求表达上的凝练；避免口语化、网络化、宣传化和模型化的空泛扩写。
正文应根据事项之间的因果、递进、并列或主次关系自然组织篇章，段落各自承担完整功能，避免把一个事项拆成连续碎片，也不要为追求形式而机械分点或强行合并不同事项。编号服务于真实层级，不能替代内容组织。
用词保持行政公文语域和适当的书面节奏，表达要简明而有分量。可在关键处自然使用四字格、对仗或排比增强概括性和节奏感，但不堆砌辞藻、不套用固定句式、不以修辞替代事实，不要求复制任何单篇范文的语言。"""
